const fs = require('fs');
const path = require('path');
const file = path.resolve(__dirname, '..', 'data', 'game_save.json');
if (!fs.existsSync(file)) { console.error('Save file not found'); process.exit(1); }
const raw = fs.readFileSync(file, 'utf8');
const bak = file + '.bak.rawreplace.' + Date.now();
fs.copyFileSync(file, bak);
let newRaw = raw.replace(/"IronOre"\s*:/g, '"IronIngot":');
newRaw = newRaw.replace(/"Ore"(?=\s*:)/g, '"Stone"');
fs.writeFileSync(file, newRaw, 'utf8');
console.log('Backup written to:', bak);
console.log('Raw replacements applied.');
